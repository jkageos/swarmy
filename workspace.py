...
if config.get("task31", {}).get("active", False):
    ...
    exp = Experiment(cfg, agent_controller, agent_sensing, Task31Environment, MyAgent)
...
if config.get("task52", {}).get("active", False):
    ...
    # allow per-task FPS override (set in config.yaml)
    cfg_base["FPS"] = t52.get("FPS", cfg_base["FPS"])

    results = {}
    completed = 0
    total = len(SWARM_SIZES) * len(SENSOR_RANGES) * EXPERIMENTS_PER_CONFIG

    print("\n" + "=" * 70)
    print("🤖 TASK 5.2: Local Sampling in a Swarm")
    print("=" * 70)
    print(f"Swarm sizes: {SWARM_SIZES}")
    print(f"Sensor ranges: {SENSOR_RANGES}")
    print(f"Experiments per configuration: {EXPERIMENTS_PER_CONFIG}")
    ...
    try:
            ...
            for r in SENSOR_RANGES:
                ...
                for i in range(EXPERIMENTS_PER_CONFIG):
                    try:
                        if (i + 1) % 100 == 0:
                            print(
                                f"  Experiment {i + 1}/{EXPERIMENTS_PER_CONFIG}...",
                                end=" ",
                                flush=True,
                            )
                        cfg = deepcopy(cfg_base)
                        ...
                        agent_controller = [SamplingController]
                        agent_sensing = [ColorSensor]
                        ...
                        agent_estimates = [
                            a.local_estimate for a in exp.world.agentlist
                        ]
                        if agent_estimates:
                        ...
                        if (i + 1) % 100 == 0:
                            ...
                            print(f"✓ ({pct:.1f}% total)", flush=True)
                    except KeyboardInterrupt:
                        raise
                    except Exception as e:
                ...
                if estimates:
                    ...
                    print(
                        f"  Mean estimate: {np.mean(estimates):.4f} ± {np.std(estimates):.4f}"
                    )
                else:
                    print(f"  ✗ No valid experiments for N={N}, r={r:.2f}")
        ...
        print(f"\n✓ Completed: {completed}/{total} experiments successful")

    except KeyboardInterrupt:
        print(
            f"\n\n✗ Interrupted by user (Ctrl+C) after {completed}/{total} experiments"
        )
        sys.exit(0)
    except Exception as e:
        print(f"\n✗ Fatal error: {e}")
