...
class BumperSensor(Perception):
    ...
    def sensor(self):
        ...
        bumper_l_pos = [
            (robot_position_x + (sensor_1_direction_x_l * 35)),
            (robot_position_y + (sensor_1_direction_y_l * 35)),
        ]
        bumper_r_pos = [
            (robot_position_x + (sensor_2_direction_x_r * 35)),
        ...
        helper_object = pygame.draw.line(
            self.agent.environment.displaySurface, bumper_color, rob_pos, bumper_l_pos
        )
        # self.environment.bumper_object_list.append(helper_object)

        # list of all objects in the environment
        objects = self.environment.get_agent_object() + [
            wall[1] for wall in self.environment.get_static_rect_list()
        ]
